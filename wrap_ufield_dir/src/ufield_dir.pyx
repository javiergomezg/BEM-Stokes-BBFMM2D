...
cdef extern from "ufield_dir.hpp":
...
def direct_ufield(np.ndarray[REAL, ndim = 1] xsrc,
			...
			Re,
			np.ndarray[REAL, ndim = 1, mode = "c"] ux_f,
			np.ndarray[REAL, ndim = 1, mode = "c"] uy_f):
	...
	cdef np.int32_t n_ux_f = len(ux_f)
	cdef np.int32_t n_uy_f = len(uy_f)

	direct_ufield_cpp(<REAL*> &xsrc[0], <int> xsrcSize,
			...
			<REAL*> &fy[0], <int> n_fy,
			<REAL> Re, 
			<REAL*> &ux_f[0], <int> n_ux_f,
			<REAL*> &uy_f[0], <int> n_uy_f)
