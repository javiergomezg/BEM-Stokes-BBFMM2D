...
H2_2D_Tree::H2_2D_Tree(const unsigned short nChebNodes, double* const charge, const vector<Point>& location, const unsigned long N, const unsigned m){
	...
	get_Standard_Chebyshev_Nodes(nChebNodes,cNode); // se calculan los nodos de chebyshev y se guardan en cNode
    //	Get Chebyshev polynomials evaluated at Chebyshev nodes
	TNode               =	MatrixXd::Zero(nChebNodes,nChebNodes);
	get_Standard_Chebyshev_Polynomials(nChebNodes,nChebNodes,cNode,TNode); //Obtiene polinomios de Chebyshev estándar evaluados en un conjunto de puntos dado y los guarda en TNodes
