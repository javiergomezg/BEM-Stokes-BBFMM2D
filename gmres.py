...
def gmres_mgs(T,S,nc,X,b,Rp,Xp,Yp,Re, R, max_iter = 1000, tol = 1.0E-8):
    ...
    aux = vector_Ax_p2p_cy(T,S,nc,X,Rp,Xp,Yp,Re)    
    r = b - aux
 ...
 #timing, kernel)
            ...
            v[:] = vector_Ax_p2p_cy(T,S,nc,vs[-1],Rp,Xp,Yp,Re)
            vs.append(v)
...
#def gmres_mgs_fmm(T,S,nc,X,b,Rp,Xp,Yp,mu, n_cheb, R, max_iter = 1000, tol = 1.0E-8):
def gmres_mgs_fmm(T,S,nc,X,b,Rp,Xp,Yp,Re, n_cheb, R = 20000, max_iter = 20000, tol = 1.0E-5):    
    #max_iter = param.max_iter
    #R = param.restart
    #tol = param.tol   
    ...
    max_outer = int(numpy.ceil(max_iter/max_inner))
    #print "max_outer:", max_outer

    # Prep for method
    #aux = gmres_dot(X, surf_array, field_array, ind0, param, timing, kernel)
    #aux = numpy.matmul(A,X)
    aux = vector_Ax_fmm(T,S,nc,X,Rp,Xp,Yp,Re,n_cheb)    
    r = b - aux
 ...
 #timing, kernel)
            #v[:] = numpy.matmul(A,vs[-1])
            T_iteration_i = time()
            v[:] = vector_Ax_fmm(T,S,nc,vs[-1],Rp,Xp,Yp,Re,n_cheb)
            time_iteration += time() - T_iteration_i
            vs.append(v)
        ...
        if rel_resid < tol:
           ...
           # print('\tTime analy: {}'.format(timing.time_an))
            del H, V, Q, g, aux, r,v,vk,vs
            return X, iteration, time_iteration/iteration
